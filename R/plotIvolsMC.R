...
plotIvolsMC <- function(ivolData, sviMatrix = NULL, slices = NULL, mcMatrix = NULL,
  ...
  for (slice in slices) {
    ...
    if (plot == TRUE) {
      plot(k, bidVol,
        col = "red", pch = 24, cex = 0.5,
        xlim = xrange, ylim = yrange, main = paste(
          "T =",
          format(t, digits = 2, nsmall = 2)
        ), xlab = "Log-Strike",
        ylab = "Implied Vol."
      )
    ...
    if ((!is.null(mcMatrix))) {
      ...
      if (plot == TRUE) {
        curve(volMC(x), from = kmin, to = kmax, col = "orange", lwd = 2, add = T)
      }
    }
    kIn <- k[!is.na(midVol)]
  ...
  par(new = F)
  return(list(
    expiries = expDates, atmVols = atmVols, atmSkew = atmSkew, atmCurv = atmCurv,
...
plotIvolsMC2 <- function(ivolData, slices = NULL, mcMatrix = NULL, mcMatrix2 = NULL,
  ...
  par(new = F)
  return(list(
    expiries = expDates, atmVols = atmVols, atmSkew = atmSkew,
