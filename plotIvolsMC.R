                         ...
                         plot = TRUE, colnum = NULL) 
{
  ...
  for (slice in slices) {
    ...
    if (plot == TRUE) { plot(k, bidVol, col = "red", pch = 24, cex = 0.5, 
    ...
    if ((!is.null(sviMatrix))) {
      ...
      if (plot == TRUE) {  curve(vol(x), from = kmin, to = kmax, col = "orange", lwd = 2, add = T)}
    }
    if ((!is.null(mcMatrix))) {
      ...
      if (plot == TRUE) {curve(volMC(x), from = kmin, to = kmax, col = "orange", lwd=2, add=T)}
    }
  kIn <- k[!is.na(midVol)]
                          ...
                          plot = TRUE, rowcol = NULL) 
{
  ...
  for (slice in slices) {
    ...
    yrange <- c(ybottom, ytop)
    if (plot == TRUE) {
      plot(k, bidVol, col = "red", pch = 24, cex = 0.5, 
           xlim = xrange, ylim = yrange, main = paste("T =", 
                                                      format(t, digits = 2, nsmall = 2)), xlab = "Log-Strike", 
           ylab = "Implied Vol.")
      points(k, askVol, col = "blue", pch = 25, cex = 0.5)
    ...
    if ((!is.null(mcMatrix))) {
      ...
      atmErrMC[slice] <- errMC(0)
      if (plot == TRUE) {
        curve(volMC(x), from = kmin, to = kmax, col = "orange", 
              lwd = 2, add = T)
      }
    }
